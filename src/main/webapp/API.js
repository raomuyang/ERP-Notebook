...
function obj_store(onSuccess, onError) {
...
function auth_role_policy_get_info_by_roleid_and_policyid(roleId, policyId, token, onSuccess, onError) {
	...
	var path = "/rest/auth/role-policy/info/roleid/{roleId}/policyid/{policyId}".replace("{roleId}", roleId).replace("{policyId}", policyId)
	$.ajax({
		url: domain + path,
		type: "GET",
		token: token,
